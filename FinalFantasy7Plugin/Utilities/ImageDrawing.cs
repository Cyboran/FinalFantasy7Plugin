...
namespace KingdomHeartsPlugin.Utilities
{
    internal static class ImageDrawing
    {
        ...
        {
            return FinalFantasy7Plugin.Tp.GetFromFile(path);
        }
        ...
        public static void DrawImageRotated(ImDrawListPtr d, ISharedImmediateTexture texture, Vector2 position, Vector2 size, float angle, uint col = UInt32.MaxValue)
        {
            ...
            var finalPosition = basePosition + position * FinalFantasy7Plugin.Ui.Configuration.Scale;
            var scaledSize = size * FinalFantasy7Plugin.Ui.Configuration.Scale;
        ...
        {
            var basePosition = ImGui.GetItemRectMin();
            var imageSize = new Vector2(image.GetWrapOrEmpty().Width, image.GetWrapOrEmpty().Height) * FinalFantasy7Plugin.Ui.Configuration.Scale;
            var finalPosition = basePosition + position * FinalFantasy7Plugin.Ui.Configuration.Scale; 
            ...
            d.AddImageQuad(image.GetWrapOrEmpty().ImGuiHandle, finalPosition + ULPos * FinalFantasy7Plugin.Ui.Configuration.Scale, finalPosition + new Vector2(imageSize.X, 0) + URPos * FinalFantasy7Plugin.Ui.Configuration.Scale, finalPosition + imageSize + LRPos * FinalFantasy7Plugin.Ui.Configuration.Scale, finalPosition + new Vector2(0, imageSize.Y) + LLPos * FinalFantasy7Plugin.Ui.Configuration.Scale, uvs[0], uvs[1], uvs[2], uvs[3], color);
            d.PopClipRect();
        }
        ...
        {
            var basePosition = ImGui.GetItemRectMin();
            var imageSize = new Vector2(image.GetWrapOrEmpty().Width, image.GetWrapOrEmpty().Height) * FinalFantasy7Plugin.Ui.Configuration.Scale * scale;
            var finalPosition = basePosition + position * FinalFantasy7Plugin.Ui.Configuration.Scale;
        ...
        {
            var basePosition = ImGui.GetItemRectMin();
            var imageSize = new Vector2(position.Z, position.W) * FinalFantasy7Plugin.Ui.Configuration.Scale;
            var finalPosition = basePosition + new Vector2(position.X, position.Y) * FinalFantasy7Plugin.Ui.Configuration.Scale;
        ...
        private static ISharedImmediateTexture? GetIconImage(uint icon, uint stackCount = 0)
        {
            ...
            return FinalFantasy7Plugin.Tp.GetFromGameIcon(lookup);
        }
